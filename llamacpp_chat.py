...
""", file=sys.stderr)
		...
		while self.remaining_tokens > 0 or self.params.interactive or self.params.n_predict == -1:
			...
			if len(self.embd) > 0:
				...
				if (self.n_past + len(self.embd) > self.n_ctx):
					print("hit ctx limit.", file=sys.stderr)
			...
			if len(self.embd_inp) <= self.input_consumed: #&& !is_interacting
				...
				logits = llama_cpp.llama_get_logits(self.ctx)#[: rows * cols]
				...
				candidates_p = llama_cpp.byref(llama_cpp.llama_token_data_array(_arr, len(_arr), False))#llama_cpp.ctypes.pointer(llama_cpp.llama_token_data_array(_arr, len(_arr), False))
				...
				llama_cpp.llama_sample_repetition_penalties(
					...
					candidates_p,
					_arr,
					last_n_repeat,#ctypes.c_size_t(last_n_repeat),
					self.params.repeat_penalty,#ctypes.c_float(self.params.repeat_penalty),
					self.params.frequency_penalty,#ctypes.c_float(self.params.frequency_penalty),
					self.params.presence_penalty,#ctypes.c_float(self.params.presence_penalty),
				)
				...
				if self.params.temp <= 0:
					...
					if self.params.mirostat == 1:
						...
						mirostat_m = 100
						llama_cpp.llama_sample_temp(self.ctx, candidates_p, ctypes.c_float(self.params.temp))
						id = llama_cpp.llama_sample_token_mirostat(self.ctx, candidates_p, ctypes.c_float(self.params.mirostat_tau), ctypes.c_float(self.params.mirostat_eta), ctypes.c_int(mirostat_m), ctypes.c_float(mirostat_mu))
					elif self.params.mirostat == 2:
						mirostat_mu = 2.0 * self.params.mirostat_tau
						llama_cpp.llama_sample_temp(self.ctx, candidates_p, ctypes.c_float(self.params.temp))
						id = llama_cpp.llama_sample_token_mirostat_v2(self.ctx, candidates_p, ctypes.c_float(self.params.mirostat_tau), ctypes.c_float(self.params.mirostat_eta), ctypes.c_float(mirostat_mu))
					else:
						# Temperature sampling
						llama_cpp.llama_sample_top_k(self.ctx, candidates_p, top_k, 1)
						# llama_cpp.llama_sample_tail_free(self.ctx, candidates_p, ctypes.c_float(self.params.tfs_z), 1)
						# llama_cpp.llama_sample_typical(self.ctx, candidates_p, ctypes.c_float(self.params.typical_p), 1)
						# llama_cpp.llama_sample_top_p(self.ctx, candidates_p, ctypes.c_float(self.params.top_p), 1)
						# llama_cpp.llama_sample_temp(self.ctx, candidates_p, ctypes.c_float(self.params.temp))
	...
	def token_to_str(self, token_id: int) -> bytes:
		...
		n = llama_cpp.llama_token_to_piece(
			self.model, llama_cpp.llama_token(token_id), buffer, size, False)
		assert n <= size
